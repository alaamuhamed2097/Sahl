...
//            try
//            {
//                var result = await PageService.GetByIdAsync(Id);
...
//            catch (Exception ex)
//            {
//                Logger.LogError(ex, "Error loading page {PageId}", Id);
...
//				else
//				{
//					ErrorMessage = result.Message ?? "Error saving page";
//				}
//			}
//			catch (Exception ex)
//			{
//				Logger.LogError(ex, "Error saving page");
//				ErrorMessage = $"Error saving page: {ex.Message}";
//			}
//			finally
//			{
//				IsSaving = false;
...
namespace Dashboard.Pages.Content.Pages
{
	public partial class Details : ComponentBase
	{
		[Parameter] public Guid? Id { get; set; } // ? ????? nullable
		...
		protected PageDto Model { get; set; } = new();
		protected bool IsSaving { get; set; } = false;
		protected bool IsAddMode => !Id.HasValue || Id.Value == Guid.Empty;
		protected string ErrorMessage { get; set; } = string.Empty;
		protected string SuccessMessage { get; set; } = string.Empty;
		...
		protected override async Task OnInitializedAsync()
		{
			if (IsAddMode)
			{
				// Add mode - Initialize empty model
				Model = new PageDto
				{
					Id = Guid.Empty,
					CreatedDateUtc = DateTime.UtcNow
		...
		protected override async Task OnAfterRenderAsync(bool firstRender)
		{
			if (firstRender)
			{
				// Give time for Quill editors to initialize
		...
		private async Task LoadPage()
		{
			...
			{
				Logger.LogError(ex, "Error loading page {Id}", Id);
				ErrorMessage = $"Error loading page: {ex.Message}";
				await Task.Delay(2000);
				Navigation.NavigateTo("/pages");
			}
		...
		protected async Task Save()
		{
			try
			{
				...
				if (result.Success)
				{
					...
					await JSRuntime.InvokeVoidAsync("swal", "Success", SuccessMessage, "success");
					await Task.Delay(1500);
					Navigation.NavigateTo("/pages");
				}
				else
				{
					ErrorMessage = result.Message ?? "Error saving page";
					await JSRuntime.InvokeVoidAsync("swal", "Error", ErrorMessage, "error");
				}
			}
			catch (Exception ex)
			{
		...
		//		PageType.RefundPolicy => "Refund Policy / ????? ?????????",
		//		PageType.ContactUs => "Contact Us / ???? ???",
		//		PageType.ReturnPolicy => "Return Policy / ????? ???????",
		//		PageType.ShippingPolicy => "Shipping Policy / ????? ?????",
		//		PageType.VendorPolicy => "Vendor Policy / ????? ??????",
		//		_ => pageType.ToString()
		//	};
		//}
		//// Helper methods
