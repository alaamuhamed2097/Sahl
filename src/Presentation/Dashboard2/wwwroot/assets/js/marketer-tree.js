// Vendor Tree JavaScript utilities
...
window.VendorTree = {
    ...
    setupTreeInteractions: function () {
        ...
        const treeContainer = document.querySelector('.Vendor-tree-container');
        if (treeContainer) {
            ...
            if (focusedNode && focusedNode.classList.contains('node-content')) {
                ...
                switch (e.key) {
                    case 'ArrowLeft':
                        ...
                        VendorTree.navigateLeft(treeNode);
                        e.preventDefault();
                        break;
                    case 'ArrowRight':
                        // Navigate to right sibling or first child
                        VendorTree.navigateRight(treeNode);
                        e.preventDefault();
                        break;
                    case 'ArrowDown':
                        // Navigate to first child
                        VendorTree.navigateDown(treeNode);
                        e.preventDefault();
                        break;
                    case 'ArrowUp':
                        // Navigate to parent
                        VendorTree.navigateUp(treeNode);
                        e.preventDefault();
                        break;
                    case 'Enter':
                    case ' ':
    ...
    centerOnNode: function (nodeElement) {
        const container = document.querySelector('.Vendor-tree-container');
...
document.addEventListener('DOMContentLoaded', function () {
    if (window.VendorTree) {
        window.VendorTree.init();
    }
});

// Export for use in Blazor
window.blazorVendorTree = {
    copyReferralLink: function (pathCode, position) {
        const link = window.VendorTree.generateReferralLink(pathCode, position);
        return window.VendorTree.copyToClipboard(link);
    },

    centerOnNode: function (pathCode) {
        const node = document.querySelector(`[data-path-code="${pathCode}"]`);
        if (node) {
            window.VendorTree.centerOnNode(node);
        }
    },

    searchNodes: function (searchTerm) {
        if (searchTerm) {
            return window.VendorTree.searchTree(searchTerm);
        } else {
            window.VendorTree.clearSearchHighlights();
            return [];
        }
    }
};