...
$(document).ready(function() {
    ...
    $(".mobile-menu").on('click', function() {
        var $this = $(this);  // Fixed: Defined $this before using it
        $this.toggleClass('on');
    });
...
function togglePcodedMenu(isCollapsed) {
...
let pcodedMenuInitialized = false;

function initializePcodedMenu() {
    if (pcodedMenuInitialized) return;

    // Use event delegation - attach one listener to the parent container
    document.addEventListener('click', function (e) {
        // Check if clicked element is a submenu trigger
        if (e.target.closest('.pcoded-hasmenu > .nav-link')) {
            e.preventDefault();

            var clickedLink = e.target.closest('.pcoded-hasmenu > .nav-link');
            var parentLi = clickedLink.closest('.pcoded-hasmenu');
            var submenu = parentLi.querySelector('.pcoded-submenu');

            if (parentLi.classList.contains('pcoded-trigger')) {
                // Close submenu
                parentLi.classList.remove('pcoded-trigger');
                submenu.style.display = 'none';
            } else {
                // Close other open submenus
                document.querySelectorAll('.pcoded-hasmenu.pcoded-trigger').forEach(function (openMenu) {
                    openMenu.classList.remove('pcoded-trigger');
                    openMenu.querySelector('.pcoded-submenu').style.display = 'none';
                });
    ...
    pcodedMenuInitialized = true;
}

function setActiveMenuItems() {
    // Your existing menu item activation logic
    $(".pcoded-navbar .pcoded-inner-navbar a").each(function() {
        var pageUrl = window.location.href.split(/[?#]/)[0];
        if (this.href == pageUrl && $(this).attr('href') != "") {
            $(this).parent('li').addClass("active");
